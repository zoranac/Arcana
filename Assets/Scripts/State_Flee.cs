...
public class State_Flee : State {
	...
	public static State_Flee Flee = new State_Flee();
	...
	public override void RunState(GameObject Owner)
	{
		if (!Owner.GetComponent<EnemyScript>().overlappingEnemy)
		{
			...
			else
			{
				...
				Owner.transform.position = Vector3.MoveTowards(Owner.transform.position, -Owner.GetComponent<EnemyScript>().targetPos, Owner.GetComponent<EnemyScript>().speed/2);
